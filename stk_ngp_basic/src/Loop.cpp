...
TEST_F(NodeLoopBenchmarking, StandardFunction) {
    AddMeshDatabase(10, 10, 10000);
    size_t num_runs = 1000;
    std::cout << std::scientific << std::setprecision(6);  // Set output to scientific notation and 6 digits of precision

    const std::array<DoubleField *, 3> fields = {velocity_field_n, acceleration_field_n, velocity_field_np1};
    NodeProcessorWithStandardFunction<3> node_processor_with_standard_function = NodeProcessorWithStandardFunction<3>(fields, bulk_data.get());
    auto start = std::chrono::high_resolution_clock::now();
...
TEST_F(NodeLoopBenchmarking, LambdaFunction) {
    AddMeshDatabase(10, 10, 10000);
    size_t num_runs = 1000;
    std::cout << std::scientific << std::setprecision(6);  // Set output to scientific notation and 6 digits of precision

    double time_inc = time_increment;

    const std::array<DoubleField *, 3> fields = {velocity_field_n, acceleration_field_n, velocity_field_np1};
    NodeProcessorWithLambdaFunction<3> node_processor_with_lambda_function = NodeProcessorWithLambdaFunction<3>(fields, bulk_data.get());
    auto start = std::chrono::high_resolution_clock::now();
    for (size_t i = 0; i < num_runs; i++) {
        node_processor_with_lambda_function.for_each_dof([&time_inc](size_t iI, std::array<double *, 3> &field_data) {
            field_data[2][iI] = field_data[0][iI] + time_inc * field_data[1][iI];
        });
    }
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed_seconds = end - start;
...
TEST_F(NodeLoopBenchmarking, StkMeshForEachEntityRun) {
    // Run the benchmarking
    AddMeshDatabase(10, 10, 10000);
    size_t num_runs = 1000;
    std::cout << std::scientific << std::setprecision(6);  // Set output to scientific notation and 6 digits of precision
